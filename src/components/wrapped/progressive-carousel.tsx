...
export const SliderIndicator: FC<SliderIndicatorProps> = ({ className, indicatorClass, activeClass }) => {
  const { active, progress, sliderValues, handleButtonClick } = useProgressSliderContext();
      ...
      {sliderValues.map((value) => {
        ...
        return (
          <button
            ...
            className={cn(
              "h-1 flex-1 rounded-full overflow-hidden bg-white/20 transition-all",
              indicatorClass
            )}
          >
            <div
              className={cn("h-full bg-white/90", activeClass)}
              style={{
